...
def predict():
    try:
      ...
      include=['float64', 'int64']).columns]) 
            ...
            if custom_model:
                custom_model_bytes = base64.b64decode(custom_model) 
                custom_model = joblib.load(io.BytesIO(custom_model_bytes)) 
...
def get_datasets_by_ids(dataset_ids):
    ...
    for dataset_id in dataset_ids:
        url = f"http://localhost:8081/dataset/{dataset_id}"  
        response = requests.get(url)
        
        if response.status_code == 200:
            dataset_content_base64 = response.json().get('content')  
            dataset_content = base64.b64decode(dataset_content_base64) 
            datasets.append(pd.read_csv(io.BytesIO(dataset_content)))  
        else:
